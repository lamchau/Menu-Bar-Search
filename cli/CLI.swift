...
struct Menu: AsyncParsableCommand {
  ...
  func run() async throws {
    ...
    if showFolders {
      let a = Alfred()
      let icon = AlfredResultItemIcon(path: "icon.settings.png")
      // settings folder
      a.add(
        .with {
    ...
    if let clickJSON = click {
      // arg format: "pid:shortcut:pathIndices:menupath"
      // e.g. "12345:⇧⌘N:2,2:File→New Canvas" or "12345::2,1:File→New Channel"
      let parts = clickJSON.split(separator: ":", maxSplits: 3, omittingEmptySubsequences: false)
      if parts.count == 4, let clickPid = Int32(parts[0]) {
        runtime.pid = clickPid
        runtime.shortcut = String(parts[1])
        runtime.pathIndices = String(parts[2])
        runtime.menuPath = String(parts[3]).split(separator: "→").map(String.init)
      }
    }
